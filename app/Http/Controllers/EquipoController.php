...
class EquipoController extends Controller
{
    ...
    function view_equi_sol(){
        ...
        $empresa = $this->select_empresa();
        return view('equipo/equi_sol', compact('trabajadores','empresa','proyectos','presupuestos','categorias'));
    }
    ...
    function view_mtto_realizados(){
        return view('equipo/mtto_realizados');
    }
    /* Combos */
    ...
    public function listar_docs(){//NO se usa
    ...
    public function guardar_seguro(Request $request){
        ...
        if (isset($file)){
            ...
            $nombre = $id_seguro.'.'.$request->nro_poliza.'.'.$file->getClientOriginalName();
    ...
    public function abrir_adjunto_seguro($file_name){
        $file_path = public_path('files/logistica/equipo_seguros/'.$file_name);
    ...
    public function listar_mttos_detalle(){
    ...
    public function mostrar_mtto($id){
        ...
        return response()->json(['mtto'=>$mtto,'sedes'=>$sedes,'grupos'=>$grupos,'areas'=>$areas]);
    }
    ...
    public function imprimir_solicitud($id_solicitud){
        $id = $this->decode5t($id_solicitud);
        $sol = DB::table('logistica.equi_sol')
        ->select('equi_sol.*','adm_area.descripcion as des_area',
        'equi_cat.descripcion as des_categoria','adm_estado_doc.estado_doc',
        'adm_documentos_aprob.id_doc_aprob','equipo.descripcion as equi_asignado',
        'equi_asig.fecha_asignacion','adm_grupo.descripcion as des_grupo',
        'sis_sede.descripcion as des_sede','adm_contri.razon_social as des_empresa',
        'proy_proyecto.descripcion as des_proyecto','equi_asig.fecha_asignacion',
        'equi_asig.fecha_inicio as fecha_inicio_asig','equi_asig.fecha_fin as fecha_fin_asig',
        'equi_asig.detalle_asignacion','sis_usua.usuario as des_usuario',
        'equi_asig.fecha_registro as fecha_registro_asig','equi_asig.id_asignacion',
        DB::raw("CONCAT(rrhh_perso.nombres,' ',rrhh_perso.apellido_paterno,' ',rrhh_perso.apellido_materno) as nombre_trabajador"))
        ->join('rrhh.rrhh_trab','rrhh_trab.id_trabajador','=','equi_sol.id_trabajador')
        ->join('rrhh.rrhh_postu','rrhh_postu.id_postulante','=','rrhh_trab.id_postulante')
        ->join('rrhh.rrhh_perso','rrhh_perso.id_persona','=','rrhh_postu.id_persona')
        ...
        if (isset($sol->id_asignacion)){
            ...
            $controles = DB::table('logistica.equi_asig_control')
                ...
                ->join('rrhh.rrhh_postu','rrhh_postu.id_postulante','=','rrhh_trab.id_postulante')
                ->join('rrhh.rrhh_perso','rrhh_perso.id_persona','=','rrhh_postu.id_persona')
                ->where([['equi_asig_control.id_asignacion','=',$sol->id_asignacion],
            ...
            foreach($controles as $d){
                ...
                <tr>
                    ...
                    <td>S/'.$d->importe.' : '.$d->galones.' gal.</td>
                </tr>
                ';
                $i++;
            }
            ...
            <body>
                ...
                <table border="0">
                    ...
                    <tr>
                        <td>Fecha Inicio</td>
                        <td width=10px>:</td>
                        <td width=300px>'.$sol->fecha_inicio_asig.'</td>
                        <td></td>
                        <td></td>
                        <td></td>
                    </tr>
                ...
                <p style="text-align:right;font-size:11px;">Fecha de Registro: '.$sol->fecha_registro_asig.'</p>
                <br/>';
            ...
            <table border="0" id="detalle">
                <thead>
                    <tr>
                        ...
                        <th width="80px">Recorrido</th>
                        <th width="200px">Chofer</th>
                        <th>Descripción</th>
                        <th>Combustible</th>
                    </tr>
                </thead>
    ...
    public function mostrar_solicitudes($id_trabajador){
        // $id_grupo = DB::table('administracion.adm_area')
        ...
        $data = DB::table('logistica.equi_sol')
            ...
            ->where([
                //  ['equi_sol.estado','=',1],
                    ['equi_sol.id_trabajador','=',$id_trabajador]])
            ->get();
        return response()->json($data);
    }
    ...
    public function guardar_asignacion(Request $request){
        ...
        if (isset($file)){
            //obtenemos el nombre del archivo
            $nombre = $id_asignacion.'.'.$equipo->codigo.'.'.$file->getClientOriginalName();
            //indicamos que queremos guardar un nuevo archivo en el disco local
    ...
    public function listar_asignaciones($id_area){
        $data = DB::table('logistica.equi_asig')
            ...
            ->join('rrhh.rrhh_postu','rrhh_postu.id_postulante','=','rrhh_trab.id_postulante')
            ->join('rrhh.rrhh_perso','rrhh_perso.id_persona','=','rrhh_postu.id_persona')
            ->where([['equi_sol.estado','=',5],
                    ['equi_sol.id_area','=',$id_area]])
            ->get();
        return response()->json($data);
    }
    ...
    public function listar_aprob_sol($rol,$id_grupo){
        $rol_concepto = DB::table('rrhh.rrhh_rol')
            ->where('id_rol',$rol)
            ->first();
        ...
        $flujos = DB::table('administracion.adm_flujo')
            ->select('adm_flujo.*')
            ->where([['adm_flujo.id_rol','=',$rol_concepto->id_rol_concepto],
                    ['adm_flujo.id_operacion','=',3], //Operacion= 3->Sol. Equipo
                    ['adm_flujo.nombre','=',$grupo->codigo],
                    ['adm_flujo.estado','=',1]
                    ])
            ->orderBy('orden')
            ->get();
        ...
        //Nro de flujos que necesita para aprobar la solicitud
        $nro_flujo = DB::table('administracion.adm_flujo')
        ->where([['adm_flujo.id_operacion','=',3],//Sol. Equipo->3
                ['adm_flujo.nombre','=',$grupo->codigo],
                ['adm_flujo.estado','=',1]//activo->1
                ])
        ->count();
