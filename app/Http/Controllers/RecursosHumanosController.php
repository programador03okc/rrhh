...
ini_set('max_execution_time', 8400);
...
class RecursosHumanosController extends Controller{
    ...
    function view_netos() {
        ...
        $trab = $this->select_personal();
        $peri = $this->select_periodo();
        return view('rrhh/remuneraciones/netos', compact('emp', 'plani', 'trab', 'peri'));
    }
    function view_encargatura() {
        $plani = $this->select_tipo_planilla();
        $emp = $this->select_empresa();
        return view('rrhh/remuneraciones/encargatura', compact('emp', 'plani'));
    }
    function view_utilidades() {
        $plani = $this->select_tipo_planilla();
        $emp = $this->select_empresa();
        $peri = $this->select_periodo();
        return view('rrhh/remuneraciones/utilidades', compact('emp', 'plani', 'peri'));
    }
    function view_beneficios() {
        $plani = $this->select_tipo_planilla();
        $emp = $this->select_empresa();
        $peri = $this->select_periodo();
        return view('rrhh/remuneraciones/beneficios', compact('emp', 'plani', 'peri'));
    }
    ...
    function view_vacaciones(){
        $empre = $this->select_empresa();
        return view('rrhh/control/vacaciones', compact('empre'));
    }
    function view_licencia(){
        $lice = $this->select_tipo_licencia();
        return view('rrhh/control/licencia', compact('lice'));
    }
    ...
    public function select_tipo_licencia(){
        $data = DB::table('rrhh.rrhh_var_licencia')->select('id_variable_licencia', 'descripcion')->orderBy('descripcion', 'asc')->where('estado', '=', 1)->get();
        return $data;
    }
    ...
    public function mostrar_contrato_trab($id){
        ...
        $data = DB::table('rrhh.rrhh_contra')
            ...
            ->where([['rrhh_contra.id_trabajador', $id], ['rrhh_contra.estado', 1]])->orderBy('fecha_inicio', 'asc')->get();
        ...
        if ($vacac->count() > 0){
            foreach ($vacac as $row){
                ...
                '<tr>
                    ...
                    <td>'.$row->dias.'</td>
                    <td><button class="btn btn-xs btn-primary" onclick="return imprimir('.$row->id_vacaciones.', '.$row->id_empresa.');"><i class="fa fa-print"></i></button></td>
                </tr>';
            }
        }else{
    ...
    public function mostrar_vacaciones_id($id){
        $vacac = DB::table('rrhh.rrhh_vacac')->where('rrhh_vacac.id_vacaciones', '=', $id)->get();
        return response()->json($vacac);
    }
    public function guardar_vacaciones(Request $request){
        ...
        if ($id > 0){
            $result = array('id_trabajador' => $request->id_trabajador, 'id_vacaciones' => $id, 'id_empresa' => $request->empresa);
        }else{
            $result = array('id_trabajador' => $request->id_trabajador, 'id_vacaciones' => 0, 'id_empresa' => $request->empresa);
        }
    ...
    public function actualizar_vacaciones(Request $request){
        ...
        if ($data > 0){
            $result = array('id_trabajador' => $request->id_trabajador, 'id_vacaciones' => $request->id_vacaciones, 'id_empresa' => $request->empresa);
        }else{
            $result = array('id_trabajador' => $request->id_trabajador, 'id_vacaciones' => 0, 'id_empresa' => $request->empresa);
        }
        return response()->json($result);
    ...
    public function mostrar_licencia_table($id){
        $html = '';
        $licencia = DB::table('rrhh.rrhh_licenc')
            ->join('rrhh.rrhh_var_licencia', 'rrhh_var_licencia.id_variable_licencia', '=', 'rrhh_licenc.id_tipo_licencia')
            ->select('rrhh_licenc.*', 'rrhh_var_licencia.descripcion AS tipo')
            ->where([['rrhh_licenc.id_trabajador', '=', $id], ['rrhh_licenc.estado', '=', 1]])->orderBy('rrhh_licenc.fecha_inicio', 'desc')->get();
    ...
    public function mostrar_licencia_id($id){
        $licencia = DB::table('rrhh.rrhh_licenc')->where('rrhh_licenc.id_licencia', '=', $id)->get();
    ...
    public function cargar_regimen($id){
        ...
        foreach ($bonif as $key){
            ...
            $sqlTP = DB::table('rrhh.rrhh_tp_plani')->where('id_tipo_planilla', $id_tp);
             if ($sqlTP->count() > 0) {
                $sqlTPla = $sqlTP->first();
                $html .= '<option value="'.$id_tp.'">'.$sqlTPla->descripcion.'</option>';
             }
        }
        return response()->json($html);
    ...
    function dias_efectivos($ini, $fin, $type){
        ...
        $cont = 0;
        $feriado = array('01/01', '28/07', '30/08', '08/10', '01/11', '25/12');
        ...
        for($i = $from; $i <= $to; $i = date("Y-m-d", strtotime($i ."+ 1 days"))){
            ...
            if (in_array($fex, $feriado) == false){
                if ($type == 'PROYECTOS QUELLAVECO') {
                    if ($dia == 0 || $dia == 1 || $dia == 2 || $dia == 3 || $dia == 4 || $dia == 5 || $dia == 6){
                        $cont += 1;
                    }
                } else if ($type == 'PROYECTOS SOPORTE') {
        ...
        if ($type == 'PROYECTOS SOPORTE') {
            $total = $cont * 8;
        } else if ($type == 'PROYECTOS QUELLAVECO') {
            $total = $cont * 12;
        } else {
            $total = $cont * 9.5;
        }
    ...
    function dias_tareo($ini, $fin, $total, $tipo = 0)
    {
        $from = date('Y-m-d', strtotime($ini));
        $to = date('Y-m-d', strtotime($fin));
        $day_final = 0;
        ...
        $day_si_lab = $this->restaFechasDias($to, $from) + 1;
        $day_no_lab = $total - $day_si_lab;

        if ($day_si_lab >= $day_no_lab) {
            $day_final = 30 - $day_no_lab;
        } else {
            $day_final = $day_si_lab;
        }
        ...
        if ($tipo > 0) {
            echo json_encode(array(
                ...
                'fecha fin de mes'  => $to,
                'dias del mes'      => $total,
                'dias laborados'    => $day_si_lab,
                'dias no laborados' => $day_no_lab,
                'dias efectivos'    => $day_final
            ));
        } else {
            return $day_final;
        }
    ...
    public function remuneracion_spcc($emp, $plani, $mes, $anio){
        ...
        foreach ($list_trab as $row){
            ...
            $sqlPensionDet = DB::table('rrhh.rrhh_pensi_detalle')
                ->where('id_pension', $ids_pnsi)
                ->where('periodo', '>=', $anio)
                ->where(function ($query) use ($mes) {
                    $query->where('mes_ini', '>=', $mes)
                      ->orWhere('mes_fin', '>=', $mes);
                })->orderBy('id_pensi_detalle', 'ASC')
                ->first();

            $psn_gral = $sqlPensionDet->porcentaje_general;
            $psn_apor = $sqlPensionDet->aporte;
            $psn_prim = $sqlPensionDet->prima_seguro;
            $psn_comi = $sqlPensionDet->comision;
            $psn_desc = ($sqlPension->descripcion == 'ONP') ? 1 : 2;

            // ASIGNACION FAMILIAR
            if ($plani == 1){
                if ($cnt_hijo == 1){
                    $sqlAsig = DB::table('rrhh.rrhh_asig_familiar')->where('fecha_inicio', '<=', $dmY_primer)->where('fecha_fin', '>=', $dmY_primer)->first();
            ...
            if ($plani == 1){
                if ($psn_desc == 1){ //ONP
                    ...
                    // $limit_afp = 9792.61;
                    // $limit_afp = 9840.89;
                    $limit_afp = DB::table('rrhh.rrhh_var_remun_aseg')->where('estado', '=', 1)->first()->valor;
            ...
            $sqlBasico = DB::table('rrhh.rrhh_aport')->where('id_variable_aportacion', 8)->where('fecha_inicio', '<=', $dmY_primer)->where('fecha_fin', '>', $dmY_primer)->first();
            $sbasico = $sqlBasico->valor;

            $dts_apor = [];
            $tot_aport = 0;
            if ($plani == 1){
                $sqlAporta = DB::table('rrhh.rrhh_aport')->where('estado', '=', 1)->where('id_variable_aportacion', '!=', 8)->get();
                ...
                foreach ($sqlAporta as $keyAporta){
                    ...
                    if ($apt_desc == 'ESSALUD (REGULAR CBSSP AGRAR/AC) TRAB') {
                        if ($tot_sueldo <= $sbasico){
                            $tot_aport = $tot_sueldo * ($sbasico / 100);
                        }else{
    ...
    /// pruebas eav
    public function info_pruebas($emp, $plani, $mes, $anio)
    {
        $dmY_primer = $this->primerDia($mes, $anio);
        $dmY_ultimo = $this->ultimoDia($mes, $anio);
        ...
        $list_trab = DB::table('rrhh.rrhh_trab')
                        ...
                        ->where( function ($querys) use ($dmY_primer, $dmY_ultimo) {
                            $querys->whereBetween('rrhh_rol.fecha_inicio', [$dmY_primer, $dmY_ultimo])
                                    ->orWhere('rrhh_rol.fecha_inicio', '<=', $dmY_primer);
                        })
                        ->orderBy('rrhh_perso.apellido_paterno', 'asc')->get();
        return json_encode($list_trab);
    }
    
    public function cargar_remuneraciones($emp, $plani, $mes, $anio, $type, $empleado, $areaGRupal, $spcc){
        $dmY_primer = $this->primerDia($mes, $anio);
        $dmY_ultimo = $this->ultimoDia($mes, $anio);
        $data_prev = array();
        $data = array();
        ...
        if ($empleado > 0) {
            $list_trab = DB::table('rrhh.rrhh_trab')
                    ...
                    ->where([['rrhh_trab.id_trabajador', '=', $empleado], ['adm_empresa.id_empresa', '=', $emp], ['rrhh_rol.id_tipo_planilla', '=', $plani],
                            ['rrhh_rol.salario', '>', 0], ['rrhh_rol.estado', '>', 0]])
                    ->get();
        }else{
            if ($type == 1){
                if ($spcc > 0) {
                    $list_trab = DB::table('rrhh.rrhh_trab')
                        ...
                        ->select(DB::raw('DISTINCT rrhh_trab.id_trabajador'), 'rrhh_perso.apellido_paterno')
                        ->where([
                            ['rrhh_trab.estado', '=', 1], ['adm_empresa.id_empresa', '=', $emp], 
                            ['rrhh_rol.id_tipo_planilla', '=', $plani], ['rrhh_rol.salario', '>', 0], 
                            ['rrhh_rol.id_proyecto', '=', 1], ['rrhh_rol.estado', '=', 1]
                        ])->orderBy('rrhh_perso.apellido_paterno', 'asc')->get();
                }else{
                    $list_trab = DB::table('rrhh.rrhh_trab')
                        ->join('rrhh.rrhh_postu', 'rrhh_postu.id_postulante', '=', 'rrhh_trab.id_postulante')
        ...
        foreach ($list_trab as $row){
            ...
            $sqlRol = DB::table('rrhh.rrhh_trab')
                ...
                ->select('rrhh_cargo.descripcion AS cargo', 'adm_area.descripcion AS area', 'sis_sede.descripcion AS sede',
                'adm_grupo.id_grupo', 'adm_grupo.descripcion AS grupo', 'rrhh_rol.salario', 'adm_contri.nro_documento AS ruc', 'adm_contri.razon_social AS empresa',
                'adm_empresa.id_empresa', 'sis_sede.id_sede', 'rrhh_rol.id_area', 'rrhh_rol.id_cargo', 'rrhh_rol.fecha_inicio', 'rrhh_rol.fecha_fin', 'rrhh_rol.sctr', 'rrhh_rol.id_proyecto')
                ->where([
                    ['rrhh_trab.id_trabajador', '=', $ids_trab],
                    ['rrhh_rol.salario', '>', 0],
                    ['adm_empresa.id_empresa', '=', $emp],
                    ['rrhh_rol.id_tipo_planilla', '=', $plani]
                ])
                ->where( function ($querys) use ($dmY_primer, $dmY_ultimo) {
                    $querys->whereBetween('rrhh_rol.fecha_inicio', [$dmY_primer, $dmY_ultimo])
                            ->orWhere('rrhh_rol.fecha_inicio', '<=', $dmY_primer);
            ...
            if ($sqlRol->count() > 0){
                ...
                $salario = $data->salario;
                $sctr = ($data->sctr == true) ? 1 : 0;
                $proyecId = ($data->id_proyecto != null) ? 1 : 0;
                // $rol_fecha_ini = date('Y-m-d', strtotime($data->fecha_inicio));
                // $rol_fecha_fin = date('Y-m-d', strtotime($data->fecha_fin));
                $rol_fecha_ini = $data->fecha_inicio;
                $rol_fecha_fin = ($data->fecha_fin != null) ? $data->fecha_fin : null;
            }else{
                $ruc = '';
                $empre = '';
                $id_empre = 0;
            ...
            $sqlContrato = DB::table('rrhh.rrhh_contra')
                ...
                ->where([['rrhh_trab.id_trabajador', '=', $ids_trab], ['rrhh_contra.estado', '>', 0]])
                ->where( function ($querys) use ($dmY_primer, $dmY_ultimo) {
                    $querys->whereBetween('rrhh_contra.fecha_inicio', [$dmY_primer, $dmY_ultimo])
                            ->orWhere('rrhh_contra.fecha_inicio', '<=', $dmY_primer);
                })
                ->limit(1)->orderBy('rrhh_contra.id_contrato', 'desc')->get();
            ...
            if ($sqlContrato->count() > 0){
                ...
                $horas = $ctts->hora_sem;
                $tp_horario = $ctts->id_horario;
                $ini_contrato = $ctts->contra_inicio;
                $fin_contrato = ($ctts->contra_fin != null) ? $ctts->contra_fin : null;

                // if ($ini_contrato <= $dmY_primer) {
                    $contarPerso = 'si';
                // }else{
                //     $contarPerso = 'no';
                // }
            ...
            $sqlPensionDet = DB::table('rrhh.rrhh_pensi_detalle')
                ...
                ->where(function ($query) use ($mes) {
                    $query->where('mes_ini', '>=', $mes)
                      ->orWhere('mes_fin', '>=', $mes);
                })->orderBy('id_pensi_detalle', 'ASC')->first();
            ...
            if ($plani == 1){
                if ($cnt_hijo == 1){
                    ...
                    //     $sqlAsig = DB::table('rrhh.rrhh_asig_familiar')->where('fecha_inicio', '>=', $dmY_primer)->where('fecha_fin', '<', $dmY_ultimo)->first();
                    //     $asg_fami = (float) $sqlAsig->valor;
                    // }
                }
            ...
            // VACACIONES
            if ($plani == 1) {
                $sqlVacac = DB::table('rrhh.rrhh_vacac')->where([['id_trabajador', $ids_trab], ['id_empresa', $emp], ['estado', 1]])
            ...
            if ($sqlLicenc->count() > 0) {
                ...
                foreach ($sqlLicenc as $keyLicen){
                    ...
                    }elseif ($keyLicen->id_tipo_licencia == 3){
                        $dias_mater += $keyLicen->dias;
                    }elseif ($keyLicen->id_tipo_licencia == 4){
                        $dias_medic += $keyLicen->dias;
                    }
                }
            }else{
                $dias_licenc = 0;
            ...
            if ($sqlDescu->count() > 0){
                foreach ($sqlDescu as $keydscto){
                    ...
                    if ($cnc_dsct == 'DONACION POR ACTIVIDAD SOCIAL' && $ids_trab == 4 && $emp == 5) {
                        $imp_dsct = 0;
                    }else{
                ...
                foreach ($sqlReten as $keyreten){
                    ...
                    if ($plani == 1) {
                        if ($cnc_reten != '4TA CATEGORIA') {
                            if (($ids_trab == 13 and $emp == 2) || ($ids_trab == 4 and $emp == 5)) {
                                $reten_con = 0;
                                $reten_sin = 0;
                            } else {
            ...
            //edgar
            if ($mes == 2) {
                if ((int) $dias_telet == $dias_total) {
                    $dias_trab_c = 0;
                    $dias_trab = ($dias_trab_c < 0) ? 0 : $dias_trab_c;
                    $dias_labor = 0;
                    $dias_lab = 0;
                    $dias_contables = 0;
                    $dias_max_cont = 0;
                    $valuex = 'no';
                    $dias_efec_lab = ($dias_labor >= $dias_total) ? $dias_total : $dias_labor;
                } else{
                    if ($fi_cont >= $dmY_primer) {
                        $dias_contables = $this->dias_tareo($fi_cont, $dmY_ultimo, $dias_total);
                    } else {
                        $dias_contables = $this->dias_tareo($dmY_primer, $dmY_ultimo, $dias_total);
                    }
                    $dias_max_cont = $dias_contables;
                    $dias_trab_c = (int) ($dias_max_cont - $min_inas - $dias_vac - $dias_licenc - $dias_telet - $dias_mater - $dias_medic); //se agrego licencias
                    $dias_trab_total = ($dias_trab_c < 0) ? 0 : $dias_trab_c;
                    $dias_trab = ($dias_trab_total > 30) ? 30 : $dias_trab_total;
                    $dias_lab = (int) $dias_total - $min_inas - $dias_vac - $dias_licenc - $dias_telet - $dias_mater - $dias_medic;
                    $dias_labor = (int) ($dias_max_cont - $min_inas - $dias_vac - $dias_licenc - $dias_telet - $dias_mater - $dias_medic);
                    $valuex = 'si';
                    $dias_efec_lab = ($dias_labor >= $dias_total) ? $dias_total : $dias_labor;
                }
            } else {
                if ((int) $dias_telet == 30) {
                    $dias_trab_c = 0;
                    $dias_trab = ($dias_trab_c < 0) ? 0 : $dias_trab_c;
                    $dias_labor = 0;
                    $dias_lab = 0;
            ...
            $dias_laborados = ($dias_labor < 0) ? 0 : $dias_labor;
            $textPru = '';
            ...
            if ($dias_lab < 0){
                ...
                if ($dias_laborados > 30) {
                    if ($fi_cont >= $dmY_primer) {
                        $horas_pre_efec = $this->dias_efectivos($fi_cont, $dmY_ultimo, $area);
                    } else {
                        $horas_pre_efec = $this->dias_efectivos($dmY_primer, $dmY_ultimo, $area);
                    }
                } else {
                    if ($fi_cont >= $dmY_primer) {
                        $horas_pre_efec = $this->dias_efectivos($fi_cont, $dmY_ultimo, $area);
            ...
            if ($dias_trab == 0) {
                $sueldo_basi = 0;
                $sueldo_asig = $asg_fami;
                if ($dias_licenc == 30) {
                    $sueldo_lice = ($dias_licenc > 0) ? (($sld_trab / 30) * $dias_licenc) : 0;
                } else {
                    if ($dias_licenc == 28 && $mes == 2) {
                        $sueldo_lice = (30 > 0) ? (($sld_trab / 30) * 30) : 0;
                    } else {
                        $sueldo_lice = ($dias_licenc > 0) ? (($sld_trab / 30) * $dias_licenc) : 0;
                    }
                }
                ...
                if ($dias_telet == 30) {
                    ...
                    if ($dias_telet == 28 && $mes == 2) {
                        $sueldo_tltb = (30 > 0) ? (($sld_trab / 30) * 30) : 0;
                    } else {
                        $sueldo_tltb = ($dias_telet > 0) ? (($sld_trab / 30) * $dias_telet) : 0;
                    }
                }
                ...
                if ($dias_mater == 30) {
                    $sueldo_lmat = ($dias_mater > 0) ? (($sld_trab / 30) * $dias_mater) : 0;
                } else {
                    $sueldo_lmat = ($dias_mater > 0) ? (($sld_trab / 30) * $dias_mater) : 0;
                }

                if ($dias_medic == 30) {
                    $sueldo_lmed = ($dias_medic > 0) ? (($sld_trab / 30) * $dias_medic) : 0;
                } else {
                    $sueldo_lmed = ($dias_medic > 0) ? (($sld_trab / 30) * $dias_medic) : 0;
                }
            } else {
            ...
            if ($anio == 2019) {
                ...
                $encargaturaSql = DB::table('rrhh.rrhh_encargatura')->where([
                    ['id_trabajador', '=', $ids_trab],
                    ['id_empresa', '=', $emp], 
                    ['id_tipo_planilla', '=', $plani],
                ])->where('fecha_inicio', '<=', $dmY_primer)->where('fecha_fin', '>=', $dmY_ultimo)
                // ->whereBetween('fecha_inicio', [$dmY_primer, $dmY_ultimo])
                ->orderBy('fecha_registro', 'DESC');

                if ($encargaturaSql->count() > 0) {
                    $encarg_colum = (float) $encargaturaSql->first()->valor;
                } else {
                    $encarg_colum = 0;
                }
            }
            ...
            $total_bonif = $bonif_con + $bonif_sin;
            $total_reint = $reint_con + $reint_sin;
            $tot_sueldo = ($sueldo_asig + ($bonif_con + $reint_con) + $encarg_colum + ($sueldo_lice + $sueldo_tltb + $sueldo_lmat + $sueldo_lmed) - ($dscto_con + $total_tndza) + $sueldo_vacac); // se agrego licencia


            ////// FORMULAS PARA BENEFICIOS
            $sqlBenef = DB::table('rrhh.rrhh_beneficios')->where([
                ['id_trabajador', '=', $ids_trab],
                ['id_empresa', '=', $emp], 
                ['id_tipo_planilla', '=', $plani],
                ['estado', '=', 1],
            ...
            if ($sqlBenef->count() > 0) {
                $bfc_cts = (float) $sqlBenef->first()->importe_cts;
                $bfc_grati = (float) $sqlBenef->first()->importe_grati;
                $bfc_vacac = (float) $sqlBenef->first()->importe_vacaciones;
                $bfc_bonif = (float) $sqlBenef->first()->importe_bonificacion;
                $bfc_desct = (float) $sqlBenef->first()->importe_descuento;
            } else {
                $bfc_cts = 0;
                $bfc_grati = 0;
                $bfc_vacac = 0;
            ...
            $total_sueldo_benef = $tot_sueldo + $bfc_vacac;
            ...
            if ($ids_trab == 101) {
                ...
                // planilla beneficios
                $tot_pensi_benef = 0;
                $dts_pensi_benef = array('tipo' => 'na', 'snp' => 0, 'spp' => 0, 'prima' => 0, 'comision' => 0, 'total_aportes' => 0);
            } else {
                if ($plani == 1){
                    if ($psn_desc == 1){ //ONP
                        $tot_pensi = $tot_sueldo * ($psn_gral / 100);
                        $dts_pensi = array('tipo' => 'onp', 'snp' => $tot_pensi, 'spp' => 0, 'prima' => 0, 'comision' => 0, 'total_aportes' => $tot_pensi);
                        // planilla beneficios
                        $tot_pensi_benef = $total_sueldo_benef * ($psn_gral / 100);
                        $dts_pensi_benef = array('tipo' => 'onp', 'snp' => $tot_pensi_benef, 'spp' => 0, 'prima' => 0, 'comision' => 0, 'total_aportes' => $tot_pensi_benef);
                    }else{
                        $limit_afp = DB::table('rrhh.rrhh_var_remun_aseg')
                                                ->where('estado', '=', 1)
                                                ->where('periodo', '>=', $anio)
                                                ->where(function ($query) use ($mes) {
                                                $query->where('mes_ini', '>=', $mes)
                        ...
                        if ($mes == 4 and $anio == 2020) {
                            ...
                            // planilla beneficios
                            $total_obli_benef = 0;
                            $total_prim_benef = ($total_sueldo_benef >= $limit_afp) ? round($limit_afp * ($psn_prim / 100), 2) : round($total_sueldo_benef * ($psn_prim / 100), 2);
                            $total_comi_benef = 0;
                        }else{
                            $total_obli = round($tot_sueldo * ($psn_apor / 100), 2);
                            $total_prim = ($tot_sueldo >= $limit_afp) ? round($limit_afp * ($psn_prim / 100), 2) : round($tot_sueldo * ($psn_prim / 100), 2);
                            ...
                            // planilla beneficios
                            $total_obli_benef = round($total_sueldo_benef * ($psn_apor / 100), 2);
                            $total_prim_benef = ($total_sueldo_benef >= $limit_afp) ? round($limit_afp * ($psn_prim / 100), 2) : round($total_sueldo_benef * ($psn_prim / 100), 2);
                            $total_comi_benef = round($total_sueldo_benef * ($psn_comi / 100), 2);
                        }

                        $tot_pensi = $total_obli + $total_prim + $total_comi;
                        $dts_pensi = array('tipo' => 'afp', 'snp' => 0, 'spp' => $total_obli, 'prima' => $total_prim, 'comision' => $total_comi, 'total_aportes' => $tot_pensi);
                        // planilla beneficios
                        $tot_pensi_benef = $total_obli_benef + $total_prim_benef + $total_comi_benef;
                        $dts_pensi_benef = array('tipo' => 'afp', 'snp' => 0, 'spp' => $total_obli_benef, 'prima' => $total_prim_benef, 'comision' => $total_comi_benef, 'total_aportes' => $tot_pensi_benef);
                    }
                }elseif ($plani == 2){
                    $tot_pensi = 0;
                    $dts_pensi = array('tipo' => 'na', 'snp' => 0, 'spp' => 0, 'prima' => 0, 'comision' => 0, 'total_aportes' => 0);
            ...
            if ($plani == 1){
                ...
                foreach ($sqlAporta as $keyAporta){
                    ...
                    if ($apt_desc == 'ESSALUD (REGULAR CBSSP AGRAR/AC) TRAB') {
                        if ($tot_sueldo <= $sbasico){
                            $tot_aport = $sbasico * ($apt_valor / 100);
                        }else{
                            $tot_aport = $tot_sueldo * ($apt_valor / 100);
                        }
                    }else{
            ...
            $netosSql = DB::table('rrhh.rrhh_neto')->where([
                ['id_trabajador', '=', $ids_trab],
                ['id_empresa', '=', $emp], 
                ['id_tipo_planilla', '=', $plani],
                ['mes', '=', $mes],
            ...
            if ($netosSql->count() > 0) {
                $neto_colum = (float) $netosSql->first()->valor;
            } else {
                $neto_colum = 0;
            }

            $SeguroSql = DB::table('rrhh.rrhh_seguro')->where([
                ['id_trabajador', '=', $ids_trab],
                ['id_empresa', '=', $emp], 
                ['id_tipo_planilla', '=', $plani],
                ['mes', '=', $mes],
            ...
            if ($SeguroSql->count() > 0) {
                $seguro_colum = (float) $SeguroSql->first()->valor;
            } else {
                $seguro_colum = 0;
            }

            $total_ingresos = $sueldo_asig + $total_reint + $total_bonif + $encarg_colum + $sueldo_vacac + $sueldo_lice + $sueldo_tltb + $sueldo_lmat + $sueldo_lmed;
            $pago_final = $sueldo_asig + $total_reint + $total_bonif + $encarg_colum + $sueldo_vacac + $sueldo_lice + $sueldo_tltb + $sueldo_lmat + $sueldo_lmed - $total_dscto - $tot_pensi - $total_tndza - $tot_reten;
            ...
            $data_prev[] = array(
                ...
                'fecha_contrato_ini'    => $ini_ctts,
                'fecha_contrato_fin'    => $fin_ctts,
                'fecha_rol_ini'         => $rol_fecha_ini,
                'fecha_rol_fin'         => $rol_fecha_fin,
                'modalidad_contrato'    => $mod_ctts,
                'tipo_contrato'         => $tip_ctts,
                'tipo_trabajador'       => $tip_trab,
                'categoria_trabajador'  => $cat_trab,
                ...
                'dias_teletrabajo'      => $dias_telet,
                'sueldo_teletrabajo'    => $sueldo_tltb,
                'dias_lic_trabajo'      => $dias_mater,
                'sueldo_lic_trabajo'    => $sueldo_lmat,
                'dias_med_trabajo'      => $dias_medic,
                'sueldo_med_trabajo'    => $sueldo_lmed,
                'aporte_pension'        => $dts_pensi,
                'pensi_aporte'          => $psn_apor,
                'pensi_prima'           => $psn_prim,
                'pensi_comision'        => $psn_comi,
                'limite_aporte'         => $limit_afp,
                'aporte_empleador'      => $dts_apor,
                'total_remun'           => $tot_sueldo,
                'total_ingreso'         => $total_ingresos,
                'total_pago'            => $pago_final,
                'sueldo_neto_val'       => $neto_colum,
                'seguro_ley'            => $seguro_colum,
                'sueldo_encargatura'    => $encarg_colum,
                'dias_mes'              => $total_dias,
                'dias_compu'            => $dias_compu,
                'regimen'               => $hora_trab,
                'hora_regimen'          => $hora_semana,
                ...
                'minutos_efectivas'     => $min_efectivos,
                'valor_final'           => $dias_contables,
                'val_dias_total'        => $dias_max_cont,
                'val_dias_trab'         => $dias_trab,
                'dias_trab_c'           => $dias_trab_c,
                'imprimir'              => $contarPerso,
                'texto_horas'           => $textPru,
                'dias_efectivos'        => $dias_efec_lab,
                'config_basico'         => $sbasico,

                'benef_cts'             => $bfc_cts,
                'benef_grati'           => $bfc_grati,
                'benef_vacaciones'      => $bfc_vacac,
                'benef_bonif'           => $bfc_bonif,
                'benef_descuent'        => $bfc_desct,
                'benef_descuent'        => $bfc_desct,
                'benef_aporte_pension'  => $dts_pensi_benef
            );
        }
        $data = ['data' =>$data_prev];
        return $data;
    ...
    public function generar_planilla_pdf($emp, $plani, $mes, $anio, $firma){
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($this->exportPlanilla($emp, $plani, $mes, $anio, 0, $firma))->setPaper('a4', 'portrait');
        return $pdf->stream('planilla.pdf');
    }

    public function generar_planilla_spcc_pdf_mes($emp, $plani, $mes, $anio){
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($this->exportPlanilla($emp, $plani, $mes, $anio, 1, 1))->setPaper('a4', 'portrait');
        return $pdf->stream('planilla.pdf');
    }
    ...
    public function procesar_planilla($emp, $plani, $mes){
        ...
        if ($insert > 0){
            $data = $this->cargar_remuneraciones($emp, $plani, $mes, $anio, 1, 0, 0, 0);
            $myData = $data['data'];
            $cont = sizeof($myData);
    ...
    public function exportPlanilla($emp, $planiPla, $mesPla, $aniPla, $spcc, $firma){
        $img = '';
        if ($firma > 0) {
            if ($emp == 1){
                $img = '<img src="./images/firma_okc.png" alt="firma-okc" height="120px">';
            }else if ($emp == 2){
                $img = '<img src="./images/firma_pyc.png" alt="firma-pyc" height="120px">';
            }else if ($emp == 3){
                $img = '<img src="./images/firma_svs.png" alt="firma-svs" height="120px">';
            }else if ($emp == 4){
                $img = '<img src="./images/firma_jdr.png" alt="firma-jdr" height="120px">';
            }else if ($emp == 5){
                $img = '<img src="./images/firma_rbd.png" alt="firma-rbd" height="120px">';
            }else if ($emp == 6){
                $img = '<img src="./images/firma_ptc.png" alt="firma-ptc" height="120px">';
            }
        }

        $data = $this->cargar_remuneraciones($emp, $planiPla, $mesPla, $aniPla, 1, 0, 0, $spcc);
        $myData = $data['data'];
        $cont = sizeof($myData);
        ...
        for ($i = 0; $i < $cont; $i++) {
            if ($myData[$i]['imprimir'] == 'si') {
                ...
                <table width="100%">
                    <tr>
                        <th colspan="2">Doc. de Identidad</th>
                    ...
                    <tr>
                        ...
                        <td align="center">ACTIVO O SUBSIDIADO</td>
                    </tr>
                    <tr>
                        <th>Fecha Ingreso</th>
                    ...
                    <tr>
                        <td align="center">'.date('d/m/Y', strtotime($myData[$i]['fecha_contrato_ini'])).'</td>
                        <td colspan="3" align="center">'.$myData[$i]['rol_trabajador'].'</td>
                        <td align="center">'.$myData[$i]['fondo_pension'].'</td>
                        <td>'.$myData[$i]['numero_cussp'].'</td>
                    </tr>
                    <tr>
                        <th rowspan="2" align="center">Remuneración Pactada</th>
                        <th colspan="2" align="center">Jornada Ord.</th>
                        <th colspan="2" align="center">Sobretiempo</th>
                        <th rowspan="2">Días Efectivos</th>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td align="center">'.number_format($myData[$i]['salario'], 2).'</td>
                        <td align="center">'.$myData[$i]['horas_efectivas'].'</td>
                        <td align="center">'.$myData[$i]['minutos_efectivas'].'</td>
                        <td align="center">0</td>
                        <td align="center">0</td>
                        <td align="center" width="10%">'.$myData[$i]['dias_efectivos'].'</td>
                    </tr>
                    <tr>
                        <th width="15%">Licencia con Goce</th>
                        <th width="15%">Licencia sin Goce</th>
                        <th width="15%">Días '.$txTele.'</th>
                        <th width="15%">Días Desc. Médico</th>
                        <th>Días Vacaciones</th>
                        <th width="10%">Días Subsidio</th>
                    </tr>
                    <tr>
                        <td align="center">'.$myData[$i]['dias_licencia'].'</td>
                        <td align="center">0</td>
                        <td align="center">'.$myData[$i]['dias_teletrabajo'].'</td>
                        <td align="center">'.$myData[$i]['dias_med_trabajo'].'</td>
                        <td align="center">'.$myData[$i]['dias_vacaciones'].'</td>
                        <td align="center">'.$myData[$i]['dias_lic_trabajo'].'</td>
                    </tr>
                </table>
                <br>
                <table width="100%">
                    ...
                    <tbody>
                        ...
                        if ($myData[$i]['dias_teletrabajo'] > 0) {
                            $total_tel = $myData[$i]['sueldo_teletrabajo'];
                        } 
                        if ($myData[$i]['dias_med_trabajo'] > 0) {
                            $total_med = $myData[$i]['sueldo_med_trabajo'];
                        }

                        $total_pag = $myData[$i]['sueldo_basico'] + $total_tel + $total_med;

                        $html .= 
                        '<tr>
                            <td width="10"></td>
                            <td>REMUNERACION BASICA</td>
                            <td align="right">'.number_format($total_pag, 2).'</td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                        ...
                        if ($myData[$i]['dias_lic_trabajo'] > 0) {
                            $html.=
                            '<tr>
                                <td width="10"></td>
                                <td>LICENCIA POR MATERNIDAD</td>
                                <td align="right">'.number_format($myData[$i]['sueldo_lic_trabajo'], 2).'</td>
                                <td></td>
                                <td></td>
                            </tr>';
                        }

                        if ($myData[$i]['sueldo_encargatura'] > 0) {
                            $html.=
                            '<tr>
                                <td width="10"></td>
                                <td>REMUNERACION POR ENCARGATURA</td>
                                <td align="right">'.number_format($myData[$i]['sueldo_encargatura'], 2).'</td>
                                <td></td>
                                <td></td>
                            </tr>';
                        }
                ...
                if ($planiPla == 1){
                    ...
                    $seguroLey = $data['data'][$i]['seguro_ley'];
        
                    for ($m = 0; $m < $contAport; $m++) {
                        if ($myAport[$m]['concepto'] == 'POLIZA DE SEGURO - D. LEG. 688') {
                            $html .=    
                            '<tr>
                                <td colspan="4">'.$myAport[$m]['concepto'].'</td>
                                <td width="15%" align="right">'.number_format($seguroLey, 2).'</td>
                            </tr>';
                        } else {
                            $html .=    
                            '<tr>
    ...
    public function exportPlanillaIndividual($emp, $planiPla, $mesPla, $aniPla, $empleado, $design, $firma){
        $img = '';
        if ($firma > 0) {
            if ($emp == 1){
                $img = '<img src="./images/firma_okc.png" alt="firma-okc" height="100px">';
            }else if ($emp == 2){
                $img = '<img src="./images/firma_pyc.png" alt="firma-pyc" height="100px">';
            }else if ($emp == 3){
                $img = '<img src="./images/firma_svs.png" alt="firma-svs" height="100px">';
            }else if ($emp == 4){
                $img = '<img src="./images/firma_jdr.png" alt="firma-jdr" height="100px">';
            }else if ($emp == 5){
                $img = '<img src="./images/firma_rbd.png" alt="firma-rbd" height="100px">';
            }else if ($emp == 6){
                $img = '<img src="./images/firma_ptc.png" alt="firma-ptc" height="100px">';
            }
        }
        $data = $this->cargar_remuneraciones($emp, $planiPla, $mesPla, $aniPla, 1, $empleado, 0, 0);
        $myData = $data['data'];
        $cont = sizeof($myData);
        $mes = $this->hallarMes($mesPla);
                    ...
                    for ($r = 0; $r < $contRet; $r++) {
                        if ($myRet[$r]['concepto'] == '5TA CATEGORIA') {
                            ...
                            <tr>
                                ...
                                <td>RENTA DE QUINTA CATEGORIA</td>
                                <td width="15%"></td>
                                <td width="15%" align="right">'.number_format($myRet[$r]['importe'], 2).'</td>
                                <td width="15%"></td>
                            </tr>';
                        } else if ($myRet[$r]['concepto'] == 'RETENCION JUDICIAL') {
                            $html .= '
                            <tr>
                                <td width="10%"></td>
                                <td>RENTENCION JUDICIAL</td>
                                <td width="15%"></td>
                                <td width="15%" align="right">'.number_format($myRet[$r]['importe'], 2).'</td>
                                <td width="15%"></td>
                            </tr>';
    ...
    public function reporte_planilla_trabajador_xls($emp, $plani, $mes, $anio, $empleado, $firma){
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($this->exportPlanillaIndividual($emp, $plani, $mes, $anio, $empleado, 1, $firma))->setPaper('a4', 'portrait');
            ...
            <table width="100%">
                ...
                <tbody>
                    ...
                    if ($myData[$i]['dias_teletrabajo'] > 0) {
                        ...
                        '<tr>
                            <td width="10"></td>
                            <td>REMUNERACION '.strtoupper($txTele).'</td>
                    ...
                    if ($myData[$i]['dias_med_trabajo'] > 0) {
                        ...
                        '<tr>
                            ...
                            <td align="right">'.number_format($myData[$i]['sueldo_med_trabajo'], 2).'</td>
                            <td></td>
                            <td></td>
                        </tr>';
                    }
    ...
    public function reporte_planilla_utilidades($emp, $plani, $anio, $firma){
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($this->pdf_utilidades($emp, $plani, $anio, 0, $firma, 0))->setPaper('a4', 'portrait');
        return $pdf->stream('reporte_utilidades.pdf');
    }

    public function pdf_utilidades($emp, $plani, $anio, $empleado, $firma, $type){
        $img = '';
        if ($firma > 0) {
            if ($emp == 1){
                $img = '<img src="./images/firma_okc.png" alt="firma-okc" height="100px">';
        ...
        if ($type == 0) {
            $sql = DB::table('rrhh.rrhh_utilidades')
                ->join('rrhh.rrhh_trab', 'rrhh_trab.id_trabajador', '=', 'rrhh_utilidades.id_trabajador')
                ->join('rrhh.rrhh_postu', 'rrhh_postu.id_postulante', '=', 'rrhh_trab.id_postulante')
                ->join('rrhh.rrhh_perso', 'rrhh_perso.id_persona', '=', 'rrhh_postu.id_persona')
                ->join('administracion.adm_empresa', 'adm_empresa.id_empresa', '=', 'rrhh_utilidades.id_empresa')
                ->join('contabilidad.adm_contri', 'adm_contri.id_contribuyente', '=', 'adm_empresa.id_contribuyente')
                ->join('administracion.adm_periodo', 'adm_periodo.id_periodo', '=', 'rrhh_utilidades.id_periodo')
                ->select(
                    'rrhh_utilidades.*', 'adm_contri.nro_documento AS ruc', 'adm_contri.razon_social AS empresa', 'rrhh_perso.nro_documento', 
                    'rrhh_perso.nombres', 'rrhh_perso.apellido_paterno', 'rrhh_perso.apellido_materno', 'adm_periodo.descripcion AS periodo'
                )
                ->where([
                    ['rrhh_utilidades.id_empresa', $emp],
                    ['rrhh_utilidades.id_tipo_planilla', $plani],
                    ['rrhh_utilidades.id_periodo', $anio]
            ...
            $sql = DB::table('rrhh.rrhh_utilidades')
                ...
                ->where([
                    ...
                    ['rrhh_utilidades.id_tipo_planilla', $plani],
                    ['rrhh_utilidades.id_periodo', $anio],
                    ['rrhh_utilidades.id_trabajador', $empleado]
                ])->get();
        }
        ...
        '<html>
            ...
            $total_ind3 = 0;
            foreach ($sql as $data) {
                $total_ind3 = $data->partic_pagar + $data->renta_quinta;
                $html .=
                '<div class="header-planilla">
                    <h2 style="text-align:center; margin: 0;">PARTICIPACION EN LAS UTILIDADES</h2>
                    <h2 style="text-align:center; margin: 0;">EJERCICIO GRAVABLE '.$data->periodo.'</h2>
                    <br>
                    <p class="text">
                        OK COMPUTER EIRL, con RUC 20519865476 domiciliado en Villa del Mar M-22, en calidad
                        de empleador y en cumplimiento de lo dispuesyo por el Decreto Legislativo No. 892 (11.11.96)
                        y el Decreto Supremo 009-98-TR (6.8.98), deja constancia de la determinación, distribución y
                        pago de la participación de las utilidades correspondientes al ejecicio '.$data->periodo.' del
                        siguiente trabajador:
                    </p>
                    <table>
                        <tbody>
                            <tr>
                                <th style="text-align: left;" width="200">Apellidos y Nombres</th>
                                <td>'.$data->apellido_paterno.' '.$data->apellido_materno.' '.$data->nombres.'</td>
                            </tr>
                            <tr>
                                <th style="text-align: left;" width="200">DNI</th>
                                <td>'.$data->nro_documento.'</td>
                            </tr>
                        </tbody>
                    </table>
                    <br><br>
                    <h2 style="text-align:center; margin: 0;">CALCULO DE LA PARTICIPACION EN LAS UTILIDADES</h2>
                    <br><br>
                    <table>
                        <tbody>
                            <tr>
                                <td colspan="3"><b>1. Utilidad a distribuir</b></td>
                                <td width="20%" style="text-align: right;">'.number_format($data->utilidad_distribuir, 2).'</td>
                            </tr>
                            <tr>
                                <td colspan="4"><b>2. Calculo de la participación</b></td>
                            </tr>
                            <tr>
                                <td width="5%"></td>
                                <td colspan="3"><b>2.1. Según los días laborados</b></td>
                            </tr>
                            <tr>
                                <td width="5%"></td>
                                <td width="5%"></td>
                                <td>* Número total de días laborados durante el Ejercicio por los Trabajadores de la empresa con derecho a percibir Utilidades</td>
                                <td style="text-align: right;">'.$data->total_dias_laborados.'</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* Número total de días laborados durante el Ejercicio por el Trabajador</td>
                                <td style="text-align: right;">'.$data->dias_laborados_trabajador.'</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* Participación del trabajador según los días laborados</td>
                                <td style="text-align: right;">'.$data->partic_trabajador_dias.'</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td colspan="3"><b>2.2. Según las remuneraciones percibidas</b></td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* Suma total de las Remuneraciones de todos los Trabajadores que corresponden al Ejercicio</td>
                                <td style="text-align: right;">'.number_format($data->suma_total_remuneracion, 2).'</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* Remuneración total del Trabajador durante el Ejercicio</td>
                                <td style="text-align: right;">'.number_format($data->total_remuneracion_trabajador, 2).'</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* Participación del trabajador según las remuneraciones percibidas</td>
                                <td style="text-align: right;">'.number_format($data->partic_trabajador_rem, 2).'</td>
                            </tr>
                            <tr>
                                <td colspan="4"><b>3. Monto de la participación a percibir por el trabajador</b></td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* Participación del trabajador en las utilidades</td>
                                <td style="text-align: right;">S/. '.number_format($total_ind3, 2).'</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* Interes</td>
                                <td style="text-align: right;">S/. '.number_format(0, 2).'</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                            ...
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* RETENCION IMPUESTO A LA RENTA</td>
                                <td style="text-align: right;">S/. '.number_format($data->renta_quinta, 2).'</td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td>* PARTICIPACION POR PAGAR</td>
                                <td style="text-align: right;">S/. '.number_format($data->partic_pagar, 2).'</td>
                            </tr>
                        </tbody>
                    </table>
                </div>';
    ...
    //nuevo
    public function generar_pdf_trabajador_mail($emp, $plani, $mes, $anio, $periodo, $trabajador, $type){
        $sql = DB::table('rrhh.rrhh_trab')
            ->join('rrhh.rrhh_postu', 'rrhh_postu.id_postulante', '=', 'rrhh_trab.id_postulante')
            ->join('rrhh.rrhh_perso', 'rrhh_perso.id_persona', '=', 'rrhh_postu.id_persona')
            ->join('rrhh.rrhh_rol', 'rrhh_rol.id_trabajador', '=', 'rrhh_trab.id_trabajador')
            ...
            ->join('administracion.sis_sede', 'sis_sede.id_sede', '=', 'adm_grupo.id_sede')
            ->join('administracion.adm_empresa', 'adm_empresa.id_empresa', '=', 'sis_sede.id_empresa')
            ->select(DB::raw('DISTINCT rrhh_trab.id_trabajador'), 'rrhh_perso.nro_documento', 'rrhh_perso.nombres', 
                'rrhh_perso.apellido_paterno', 'rrhh_perso.apellido_materno', 'rrhh_postu.correo')
            ->where([
                ['rrhh_trab.id_trabajador', '=', $trabajador],
                ['rrhh_trab.estado', '=', 1],
                ['adm_empresa.id_empresa', '=', $emp],
                ['rrhh_rol.id_tipo_planilla', '=', $plani], 
                ['rrhh_rol.salario', '>', 0],
                ['rrhh_rol.estado', '=', 1], 
                // ['rrhh_postu.correo', '!=', null]
            ])
            ->orderBy('rrhh_perso.apellido_paterno', 'asc')->get();
        ...
        foreach ($sql as $key){
            ...
            $name = $key->apellido_paterno.' '.$key->apellido_materno.' '.$key->nombres;
            $nameFile = $dni.'_'.$mes.'_'.$anio;
            
            if ($type == 'planilla') {
                $enviar = $this->enviar_email($emp, $plani, $mes, $anio, $empleado, $correo, $nameFile, $name);
            } else if ($type == 'utilidades') {
                $enviar = $this->enviar_email_utilidades($emp, $plani, $periodo, $empleado, $correo, $nameFile, $name);
            }
        }
        $data = array('name' => $name, 'response' => $enviar);
        return json_encode($data);
    }

    //ealvarez777
    public function enviar_correos(Request $request) {
        $empresa = (int) $request->data_empresa;
        $planilla = (int) $request->data_tipo_planilla;
        $mes = (int) $request->data_mes;
        $periodo = (int) $request->data_periodo;
        $empleado = $request->id_trabajador;
        ...
        for($i = 0; $i < $count; $i++){
            $enviar = $this->generar_pdf_trabajador_mail($empresa, $planilla, $mes, $periodo, 0, $empleado[$i], 'planilla');
            $respuesta = json_decode($enviar);

            if ($respuesta->response == 'si') {
                $recib[] = $respuesta->name;
            } else {
                $error[] = $respuesta->name;
            }
        }
        return view('rrhh/remuneraciones/email', compact('recib', 'error'));
    }

    public function enviar_correos_utilidades(Request $request) {
        $empresa = (int) $request->email_data_empresa;
        $planilla = (int) $request->email_data_tipo_planilla;
        $periodo = (int) $request->email_data_periodo;
        $id_periodo = (int) $request->email_data_id_periodo;
        $empleado = $request->id_trabajador;
        $count = count($empleado);
        ...
        for($i = 0; $i < $count; $i++){
            $enviar = $this->generar_pdf_trabajador_mail($empresa, $planilla, 'utilidades', $periodo, $id_periodo, $empleado[$i], 'utilidades');
            $respuesta = json_decode($enviar);
    ...
    public function enviar_email($emp, $plani, $mes, $anio, $empleado, $correo, $nameFile, $datos){
        $data = $this->exportPlanillaIndividual($emp, $plani, $mes, $anio, $empleado, 1, 1);
        $imgs = $this->exportPlanillaIndividual($emp, $plani, $mes, $anio, $empleado, 0, 1);
        ...
        if ($emp == 1) { //office365 okc
            $smtpAddress = 'smtp.office365.com';
            $port = 587;
            $encryption = 'tls';
            $yourEmail = 'boletasdepago@okcomputer.com.pe';
            $yourPassword = 'Inicio01.';
        } else if ($emp == 2) { //zoho pyc
            $smtpAddress = 'smtp.zoho.com';
            $port = 465;
            $encryption = 'ssl';
            $yourEmail = 'rrhh@proyectec.com.pe';
            $yourPassword = 'RRhh2020@';
        } else if ($emp == 3) { //office365 svs
            $smtpAddress = 'smtp.office365.com';
            $port = 587;
            $encryption = 'tls';
            $yourEmail = 'rrhh@smartvalue.com.pe';
            $yourPassword = 'S976503v';
        } else if ($emp == 5) { //gmail rbd
            $smtpAddress = 'smtp.gmail.com';
            $port = 465;
            $encryption = 'ssl';
            $yourEmail = 'doradorrhh2020@gmail.com.pe';
            $yourPassword = '994798168';
        } else if ($emp == 6) { //gmail ptc
            $smtpAddress = 'smtp.gmail.com';
            $port = 465;
            $encryption = 'ssl';
            $yourEmail = 'rrhh.protecnologia@gmail.com.pe';
            $yourPassword = '605551590';
        }
        ...
        $message = (new Swift_Message('Boleta de Pagos'))
            ...
            // ->addCc('rrhh@okcomputer.com.pe') CON COPIA
            ->setTo([$correo])
    ...
    public function enviar_email_utilidades($emp, $plani, $periodo, $empleado, $correo, $nameFile, $datos){
        $data = $this->pdf_utilidades($emp, $plani, $periodo, $empleado, 1, 1);
        $imgs = $this->pdf_utilidades($emp, $plani, $periodo, $empleado, 0, 1);
        $ruta = '/utilidades/'.$nameFile.'.pdf';
        $atach = asset('files').$ruta;
        $pdf = \App::make('dompdf.wrapper');
        $pdf->setPaper('a4', 'portrait');
        $pdf->loadHTML($data);
    ...
    public function reporte_gastos($planiRem, $mesRem, $anio){
        ...
        foreach ($dataEmp as $key){
            $data = $this->cargar_remuneraciones($key->id_empresa, $planiRem, $mesRem, $anio, 1, 0, 0, 0);
            $myData = $data['data'];
            $cont = sizeof($myData);
            ...
            for ($i = 0; $i < $cont; $i++){
                ...
                $ficon = $myData[$i]['fecha_contrato_ini'];
                ...
                $salud = ($remun*0.09);
                $viley = ($remun*0.0053);
                $total = ($ingre + $vacac + $grati + $micts + $salud + $tsctr + $viley);
                $costo = ($ingre > 0) ? ($total / $ingre) : 0;
                
                $html.=
                '<tr>
                    <td>'.$traba.'</td>
                    <td>'.$cargo.'</td>
                    <td>'.date('d/m/Y', strtotime($ficon)).'</td>
                    <td>'.$ccost.'</td>
                    <td>'.$tpcos.'</td>
                    ...
                    <td class="okc-numero">'.number_format($viley, 2).'</td>
    ...
    public function resumen_planilla($planiRem, $mesRem, $anio){
        $dataEmp = DB::table('administracion.adm_empresa')->select('id_empresa')->where('estado', '=', 1)->orderBy('id_empresa', 'ASC')->get();
        $mes = $this->hallarMes($mesRem);

        $codeEmp = '';
        $head = '<tr><td></td>';
        foreach ($dataEmp as $headx) {
            $empresa = DB::table('administracion.adm_empresa')->select('codigo')->where('id_empresa', '=', $headx->id_empresa)->get();
            $codeEmp = $empresa->first()->codigo;
            $head .= '<td>'.$codeEmp.'</td>';
        }
        $head .= '</tr>';
        
        $arrayArea = array('ADMINISTRACION', 'COMERCIAL', 'GERENCIA', 'PROYECTOS');
        $html = '';

        foreach ($arrayArea as $areax) {
            $html .= '<tr><td>'.$areax.'</td>';
            foreach ($dataEmp as $key) {
                $data = $this->cargar_remuneraciones($key->id_empresa, $planiRem, $mesRem, $anio, 1, 0, $areax, 0);
                $myData = $data['data'];
                $cont = sizeof($myData);
                $subtotal = 0;
                ...
                for ($i = 0; $i < $cont; $i++) {
                    $ingre = $myData[$i]['total_ingreso'];
                    $grupo = $myData[$i]['grupo'];
                    $areaf = $myData[$i]['area'];
                    
                    $subtotal = $subtotal + $ingre;
                }
                $html .= '<td>'.$subtotal.'</td>';
            }
            $html .= '</tr>';
        ...
        $header = $head;
        $export = $html;
        return view('rrhh/reportes/Resumen_Planilla', compact('header', 'export'));
    }
    ...
    public function reporte_planilla_grupal_xls($planiRem, $mesRem, $anio, $grupo){
        ...
        foreach ($dataEmp as $key){
            $data = $this->cargar_remuneraciones($key->id_empresa, $planiRem, $mesRem, $anio, 3, 0, $grupo, 0);
            $myData = $data['data'];
            $cont = sizeof($myData);
            $name_empresa = DB::table('administracion.adm_empresa')->select('codigo')->where('id_empresa', '=', $key->id_empresa)->get();
            $codeEmp = $name_empresa->first()->codigo;
            ...
            if ($planiRem == 1){
                ...
                <tr>
                    ...
                    <th rowspan="2">Remun. Teletrabajo</th>
                    ...
                    <th rowspan="2" width="80">Descuento Autorizado</th>
            ...
            <tbody>';
                ...
                for ($i = 0; $i < $cont; $i++){
                    ...
                    $fing = date('d/m/Y', strtotime($myData[$i]['fecha_contrato_ini']));
                    ...
                    $teletrabajo = $myData[$i]['sueldo_teletrabajo'];
                    ...
                    $myRetencJud = $data['data'][$i]['retenciones'];
                    $countRetencJud = sizeof($myRetencJud);
                    $judic = 0;
                    for ($retjud = 0; $retjud < $countRetencJud; $retjud++){ 
                        if ($myRetencJud[$retjud]['concepto'] == 'RETENCION JUDICIAL'){
                            $judic = $myRetencJud[$retjud]['importe'];
                        }
                    }
                    ...
                    $myDesctos = $data['data'][$i]['descuentos'];
                    $countDesctos = sizeof($myDesctos);
                    $prest = 0;
                    $otrod = 0;
                    $adela = 0;
                    $donac = 0;
                    $inasi = 0;
                    for ($ptm = 0; $ptm < $countDesctos; $ptm++) { 
                        if ($myDesctos[$ptm]['concepto'] == 'PRESTAMOS'){
                            $prest += $myDesctos[$ptm]['importe'];
                        }elseif($myDesctos[$ptm]['concepto'] == 'OTROS DESCUENTOS NO DEDUC. DE BASE IMPONIBLE'){
                            $otrod += $myDesctos[$ptm]['importe'];
                        }elseif($myDesctos[$ptm]['concepto'] == 'DONACION POR ACTIVIDAD SOCIAL'){
                            $donac += $myDesctos[$ptm]['importe'];
                        }elseif($myDesctos[$ptm]['concepto'] == 'ADELANTOS'){
                            $adela += $myDesctos[$ptm]['importe'];
                        }elseif ($myDesctos[$ptm]['concepto'] == 'INASISTENCIAS'){
                            $inasi += $myDesctos[$ptm]['importe'];
                        }
                    }
                    ...
                    $descuents = $monTa + $inasi;
                    $dsctos_no = $prest + $otrod + $adela + $judic + $donac;
                    $neto = $myData[$i]['total_pago'];
                    $vacac = $myData[$i]['sueldo_vacaciones'];
                    $netoCal = $myData[$i]['sueldo_neto_val'];
                    ...
                    '<tr>
                        ...
                        <td class="okc-numero">'.number_format($teletrabajo, 2).'</td>
                        ...
                        <td class="okc-numero">0</td>
                        <td class="okc-numero">'.number_format($inasi, 2).'</td>
                        <td class="okc-numero">0</td>
                        <td class="okc-numero">0</td>
                        ...
                        <td class="okc-numero">'.number_format($judic, 2).'</td>
                        <td class="okc-numero">'.number_format(0, 2).'</td>
                        <td class="okc-numero">'.number_format($donac, 2).'</td>
                        ...
                        <td class="okc-numero">'.number_format($netoCal, 2).'</td>
                    </tr>';
                }
            }else if ($planiRem == 2){
                $html .=
    ...
    public function reporte_planilla_xls($emp, $planiRem, $mesRem, $anio, $filter, $valueGroup){
        $data = $this->cargar_remuneraciones($emp, $planiRem, $mesRem, $anio, 1, 0, 0, 0);    
        $myData = $data['data'];
        $cont = sizeof($myData);
        $mes = $this->hallarMes($mesRem);
        $thead = '';
        ...
        if ($planiRem == 1){
            ...
            '<tr>
                ...
                <th rowspan="3">Remun. Básica</th>
                <th colspan="9">CONCEPTOS BASICOS NO REMUNERATIVOS</th>
                <th colspan="11">REMUNERACION ASEGURABLE</th>
                <th rowspan="3">Total Ingresos</th>
                <th rowspan="3">Total Remun. Asegurable</th>
                <th colspan="5">APORTES</th>
                <th rowspan="3">Total Aportes</th>
            ...
            <tr>
                ...
                <th rowspan="2">Remun. Principal</th>
                <th rowspan="2">Remun. Teletrabajo</th>
                <th rowspan="2">Remun. Encargatura</th>
                <th rowspan="2">Asig. Familiar</th>
                <th colspan="3">Horas Extras</th>
                <th rowspan="2">Reintegro</th>
                <th colspan="3">Vacaciones</th>
            ...
            for ($i = 0; $i < $cont; $i++){
                ...
                $sueldo = $myData[$i]['sueldo_basico'];
                $teletrabajo = $myData[$i]['sueldo_teletrabajo'];
                $encargatura = $myData[$i]['sueldo_encargatura'];
                $total_remun = $myData[$i]['total_remun'];
                ...
                '<tr>
                    ...
                    <td class="okc-numero">'.number_format($sueldo, 2).'</td>
                    <td class="okc-numero">'.number_format($teletrabajo, 2).'</td>
                    <td class="okc-numero">'.number_format($encargatura, 2).'</td>
                    <td class="okc-numero">'.number_format($asig, 2).'</td>
                    <td class="okc-numero">0</td>
                    <td class="okc-numero">0</td>
                    <td class="okc-numero">0</td>
            ...
            for ($i = 0; $i < $cont; $i++){
                ...
                '<tr>
                    ...
                    <td class="okc-numero">'.number_format(round($monTa), 2).'</td>
                    <td class="okc-numero">'.number_format(0).'</td>
                    <td class="okc-numero">'.number_format(round($inasi), 2).'</td>
                    <td class="okc-numero">'.number_format($dscto, 2).'</td>
                    <td class="okc-numero">'.number_format(round($neto), 2).'</td>
                    <td class="okc-numero">'.$horas.'</td>
                    <td class="okc-numero">'.$mints.'</td>
                    <td class="okc-numero">'.number_format(round($netoCal), 2).'</td>
                </tr>';
            }
        }
    ...
    public function reporte_planilla_benef_xls($emp, $planiRem, $mesRem, $anio, $filter, $valueGroup){
        $data = $this->cargar_remuneraciones($emp, $planiRem, $mesRem, $anio, 1, 0, 0, 0);    
        $myData = $data['data'];
        $cont = sizeof($myData);
        $mes = $this->hallarMes($mesRem);
        ...
        if ($planiRem == 1){
            ...
            '<tr>
                ...
                <th colspan="2">Horas Efectivas</th>
                <th rowspan="3">Neto Cálculo</th>
                <th colspan="9">NUEVOS CALCULOS DE PLANILLA</th>
            </tr>
            <tr>
                <th colspan="3">LIQUIDACION</th>
                <th rowspan="2" width="80">Reint. No Deduc.</th>
                ...
                <th rowspan="2">SNP 13%</th>
                <th colspan="3">SPP</th>
                <th rowspan="2" width="80">CTS</th>
                <th rowspan="2" width="80">Gratif.</th>
                <th rowspan="2" width="80">Vacaciones Truncas</th>
                <th rowspan="2" width="80">Bonific. ExtraOrd.</th>
                <th rowspan="2" width="80">Otros Dsctos.</th>
            </tr>
            <tr>
                <th>N° 25%</th>
                <th>N° 35%</th>
            ...
            for ($i = 0; $i < $cont; $i++){
                ...
                // PLANILLA BENEFICIOS
                $myPensiBenef = $data['data'][$i]['benef_aporte_pension'];
                $tpPensi = $myPensiBenef['tipo'];
                if ($tpPensi == 'onp') {
                    $snp_bfc = $myPensiBenef['snp'];
                    $spp_bfc = 0;
                    $prima_bfc = 0;
                    $comision_bfc = 0;
                }else{
                    $snp_bfc = 0;
                    $spp_bfc = $myPensiBenef['spp'];
                    $prima_bfc = $myPensiBenef['prima'];
                    $comision_bfc = $myPensiBenef['comision'];
                }

                // PLANILLA BENEFICIOS
                $bfc_cts = $myData[$i]['benef_cts'];
                $bfc_grati = $myData[$i]['benef_grati'];
                $bfc_vacac = $myData[$i]['benef_vacaciones'];
                $bfc_bonif = $myData[$i]['benef_bonif'];
                $bfc_dscto = $myData[$i]['benef_descuent'];
                ...
                '<tr>
                    ...
                    <td class="okc-numero">'.number_format($snp_bfc, 2).'</td>
                    <td class="okc-numero">'.number_format($spp_bfc, 2).'</td>
                    <td class="okc-numero">'.number_format($prima_bfc, 2).'</td>
                    <td class="okc-numero">'.number_format($comision_bfc, 2).'</td>
                    <td class="okc-numero">'.number_format($bfc_cts, 2).'</td>
                    <td class="okc-numero">'.number_format($bfc_grati, 2).'</td>
                    <td class="okc-numero">'.number_format($bfc_vacac, 2).'</td>
                    <td class="okc-numero">'.number_format($bfc_bonif, 2).'</td>
                    <td class="okc-numero">'.number_format($bfc_dscto, 2).'</td>
                </tr>';
            }
        }else if ($planiRem == 2){
            $thead .=
        ...
        $perExport = 'PERIODO '.$mes.'-'.$anio;
        $companyExport = $codeEmp;
        return view('rrhh/reportes/Planilla_Remuneracion_Benef', compact('headExport', 'dataExport', 'perExport', 'companyExport'));
    }
    ...
    public function reporte_planilla_spcc_xls_mes($emp, $planiRem, $mesRem, $anio){
        $data = $this->cargar_remuneraciones($emp, $planiRem, $mesRem, $anio, 1, 0, 0, 1);    
        $myData = $data['data'];
        $cont = sizeof($myData);
        $mes = $this->hallarMes($mesRem);
        $thead = '';
    ...
    public function generar_vacaciones_pdf($id, $emp){
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($this->exportVacaciones($id, $emp))->setPaper('A5', 'portrait');
        return $pdf->stream('reporte.pdf');
    }

    public function exportVacaciones($id, $emp){
        $sql = DB::table('rrhh.rrhh_vacac')->where('id_vacaciones', $id)->get();

        $img = '';
        if ($emp == 1){
            $img = '<img src="./images/logo_okc.png" alt="logo-okc" height="30px">';
        }else if ($emp == 2){
            $img = '<img src="./images/logo_proyectec.png" alt="logo-pyc" height="30px">';
        }else if ($emp == 3){
            $img = '<img src="./images/logo_smart.png" alt="logo-svs" height="30px">';
        }else if ($emp == 4){
            $img = '<img src="./images/logo_deza.png" alt="logo-jdr" height="30px">';
        }else if ($emp == 5){
            $img = '<img src="./images/logo_rbd.png" alt="logo-rbd" height="30px">';
        }else if ($emp == 6){
            $img = '<img src="./images/logo_protecnologia.png" alt="logo-ptc" height="30px">';
        }
        ...
        foreach ($sql as $row){
            ...
            $empreData = DB::table('administracion.adm_empresa')
                            ->join('contabilidad.adm_contri', 'adm_contri.id_contribuyente', '=', 'adm_empresa.id_contribuyente')
                            ->select('adm_contri.razon_social AS empresa', 'adm_contri.nro_documento AS documento')
                            ->where('adm_empresa.id_empresa', $emp)->first();
            $documento = $empreData->documento;
            $empresa = $empreData->empresa;
        }
            ...
            <tbody>
                <tr>
                    <td width="90">
                        ...
                        <p class="title">RUC: '.$documento.'</p>
                    </td>
                    <td class="title center">PAPELETA DE VACACIONES</td>
                    <td width="90" align="center">'.$img.'</td>
                </tr>
            </tbody>
        </table>
        <br>
    ...
    public function reporte_afp(){
        ...
        $data = $this->cargar_remuneraciones(0, 1, 9, $anio, 2, 0, 0, 0);
        $myData = $data['data'];
        $cont = sizeof($myData);
    ...
    // ealvarez777
    public function load_trabajadores_sueldo(Request $request){
        $emp = $request->empresa;
        $plani = $request->planilla;
        $type = $request->movim;
        $mes = $request->mes;
        $periodo = $request->periodo;
        $html = '';
        $dmY_primer = $this->primerDia($mes, $periodo);
        ...
        if ($type == 0) {
            $list_trab = DB::table('rrhh.rrhh_trab')
                    ...
                    ->join('administracion.sis_sede', 'sis_sede.id_sede', '=', 'adm_grupo.id_sede')
                    ->join('administracion.adm_empresa', 'adm_empresa.id_empresa', '=', 'sis_sede.id_empresa')
                    ->select(DB::raw('DISTINCT rrhh_trab.id_trabajador'), 'rrhh_perso.apellido_paterno', 'rrhh_perso.apellido_materno', 'rrhh_perso.nombres')
                    ->where([
                        ['rrhh_trab.estado', '>=', 1], ['adm_empresa.id_empresa', '=', $emp], ['rrhh_rol.id_tipo_planilla', '=', $plani],
                        ['rrhh_rol.salario', '>', 0], ['rrhh_rol.estado', '>', 0]
                    ])
            ...
            $list_trab = DB::table('rrhh.rrhh_trab')
                    ...
                    ->join('administracion.sis_sede', 'sis_sede.id_sede', '=', 'adm_grupo.id_sede')
                    ->join('administracion.adm_empresa', 'adm_empresa.id_empresa', '=', 'sis_sede.id_empresa')
                    ->select(DB::raw('DISTINCT rrhh_trab.id_trabajador'), 'rrhh_perso.nro_documento', 'rrhh_perso.apellido_paterno', 'rrhh_perso.apellido_materno',
                            'rrhh_perso.nombres', 'rrhh_postu.correo')
                    ->where([
                        ['rrhh_trab.estado', '=', 1], ['adm_empresa.id_empresa', '=', $emp],
                        ['rrhh_rol.id_tipo_planilla', '=', $plani], ['rrhh_rol.salario', '>', 0], 
                        ['rrhh_rol.estado', '=', 1], ['rrhh_postu.correo', '!=', null]
                    ])
                    ->orderBy('rrhh_perso.apellido_paterno', 'asc')->get();
        } else if ($type == 4) {
            $list_trab = DB::table('rrhh.rrhh_trab')
                    ...
                    ->join('administracion.sis_sede', 'sis_sede.id_sede', '=', 'adm_grupo.id_sede')
                    ->join('administracion.adm_empresa', 'adm_empresa.id_empresa', '=', 'sis_sede.id_empresa')
                    ->select(DB::raw('DISTINCT rrhh_trab.id_trabajador'), 'rrhh_perso.nro_documento', 
                            'rrhh_perso.apellido_paterno', 'rrhh_perso.apellido_materno', 'rrhh_perso.nombres', 'rrhh_postu.correo')
                    ->where([
                        ['rrhh_trab.estado', '>=', 1], ['adm_empresa.id_empresa', '=', $emp], 
                        ['rrhh_rol.id_tipo_planilla', '=', $plani], ['rrhh_rol.salario', '>', 0], 
                        ['rrhh_rol.estado', '>', 0], ['rrhh_postu.correo', '!=', null]
                    ])
                    ->orderBy('rrhh_perso.apellido_paterno', 'asc')->get();
        } else {
            $list_trab = DB::table('rrhh.rrhh_trab')
        ...
        if (count($list_trab) > 0) {
            $array = array('response' => 'ok', 'data' => $list_trab);
        } else {
            $array = array('response' => 'null');
        }
        return response($array, 200);
    }

    public function load_trabajadores_benefecios(Request $request){
        $emp = $request->empresa;
        $plani = $request->planilla;
        $type = $request->movim;
        $mes = $request->mes;
        ...
        if (count($list_trab) > 0) {
            $arrayNew = [];
            foreach ($list_trab as $key) {
                $id_trab = $key->id_trabajador;
                $nom_tra = $key->nombres.' '.$key->apellido_paterno;
                ...
                $query = DB::table('rrhh.rrhh_beneficios')->where([
                    ['id_trabajador', $id_trab], ['id_empresa', $emp], ['id_tipo_planilla', $plani], ['periodo', $periodo], ['mes', $mes], ['estado', 1]
                ])->orderBy('id_beneficio', 'desc')->first();

                if ($query != null) {
                    $val_cts = $query->importe_cts;
                    $val_gra = $query->importe_grati;
                    $val_vac = $query->importe_vacaciones;
                    $val_bon = $query->importe_bonificacion;
                    $val_dct = $query->importe_descuento;
                }
                $arrayNew[] = array(
                    'id_trabajador'     => $id_trab,
                    'dts_trabajador'    => $nom_tra,
                    'cts'               => $val_cts,
                    'gratificacion'     => $val_gra,
                    'vacaciones'        => $val_vac,
                    'bonificacion'      => $val_bon,
                    'descuento'         => $val_dct
                );
            }
            $array = array('response' => 'ok', 'data' => $arrayNew);
        } else {
            $array = array('response' => 'null');
        }
        return response($array, 200);
    ...
    public function guardar_netos(Request $request) {
        $empresa = $request->data_empresa;
        $planilla = $request->data_tipo_planilla;
        $mes = $request->data_mes;
        $periodo = $request->data_periodo;
        $trabajador = $request->id_trabajador;
        $sueldo = $request->sueldo;
        $fecha_registro = date('Y-m-d H:i');

        $count = count($trabajador);

        for($i = 0; $i < $count; $i++){
            DB::table('rrhh.rrhh_neto')->insertGetId(
                [
                    'id_empresa'        => $empresa,
                    'id_tipo_planilla'  => $planilla,
                    'mes'               => $mes,
                    'periodo'           => $periodo,
                    'id_trabajador'     => $trabajador[$i],
                    'valor'             => $sueldo[$i],
                    'estado'            => 1,
                    'fecha_registro'    => $fecha_registro
                ],
        ...
        return redirect()->back()->with('success', 'Se registraron los sueldos netos');   
    }
    ...
    public function guardar_seguro(Request $request) {
        ...
        for($i = 0; $i < $count; $i++){
            if ($valor[$i] > 0) {
                DB::table('rrhh.rrhh_seguro')->insertGetId(
                    [
                        'id_empresa'        => $empresa,
                        'id_tipo_planilla'  => $planilla,
                        'mes'               => $mes,
        ...
        return redirect()->back()->with('success', 'Se registraron los seguros de ley netos');   
    }

    public function guardar_encargatura(Request $request) {
        $empresa = $request->data_empresa;
        $planilla = $request->data_tipo_planilla;
        $fechaini = $request->data_fecha_ini;
        $fechafin = $request->data_fecha_fin;
        $trabajador = $request->id_trabajador;
        $sueldo = $request->sueldo;
        $fecha_registro = date('Y-m-d H:i');
        ...
        for($i = 0; $i < $count; $i++){
            if ($sueldo[$i] > 0) {
                DB::table('rrhh.rrhh_encargatura')->insertGetId(
                    [
                        'id_empresa'        => $empresa,
                        'id_tipo_planilla'  => $planilla,
        ...
        return redirect()->back()->with('success', 'Se registraron las remuneraciones por encargatura');   
    }

    public function guardar_utilidades(Request $request) {
        $empresa = $request->data_empresa;
        $planilla = $request->data_tipo_planilla;
        $periodod = $request->data_periodo;
        $util_dis = $request->data_utilidad_dist;
        $todias_t = $request->data_total_dias_lab;
        $suto_rem = $request->data_sum_total_remun;

        $trabajador = $request->id_trabajador;
        $dias_lab_t = $request->dias_lab_trabajador;
        $dias_par_t = $request->dias_parti_trabajador;
        $rem_tota_t = $request->remun_total_trabajador;
        $rem_part_t = $request->remun_parti_trabajador;
        $rem_part_p = $request->remun_parti_pagar;
        $quinta_cat = $request->renta_quinta;
        $fecha_registro = date('Y-m-d H:i');
        ...
        for($i = 0; $i < $count; $i++) {
            if ($dias_lab_t[$i] > 0) {
                DB::table('rrhh.rrhh_utilidades')->insertGetId(
                    [
                        'id_empresa'                    => $empresa,
                        'id_tipo_planilla'              => $planilla,
                        'id_periodo'                    => $periodod,
                        'id_trabajador'                 => $trabajador[$i],
                        'utilidad_distribuir'           => $util_dis,
                        'total_dias_laborados'          => $todias_t,
                        'suma_total_remuneracion'       => $suto_rem,
                        'dias_laborados_trabajador'     => $dias_lab_t[$i],
                        'partic_trabajador_dias'        => $dias_par_t[$i],
                        'total_remuneracion_trabajador' => $rem_tota_t[$i],
                        'partic_trabajador_rem'         => $rem_part_t[$i],
                        'partic_pagar'                  => $rem_part_p[$i],
                        'renta_quinta'                  => $quinta_cat[$i],
                        'estado'                        => 1,
                        'fecha_registro'                => $fecha_registro
                    ],
                    'id_utilidad'
        ...
        return redirect()->back()->with('success', 'Se registraron las utilidades correctamente');   
    }
    ...
    public function guardar_beneficios(Request $request) {
        ...
        $trabajador = $request->id_trabajador;
        $imp_cts = $request->cts;
        $imp_grati = $request->grati;
        $imp_vacac = $request->truncas;
        $imp_bonif = $request->bonif;
        $imp_desct = $request->descuento;
        $fecha_registro = date('Y-m-d H:i');

        $count = count($trabajador);

        for($i = 0; $i < $count; $i++) {
            $val_totales = $imp_cts[$i] + $imp_grati[$i] + $imp_vacac[$i] + $imp_bonif[$i] + $imp_desct[$i];
            if ($val_totales > 0) {
                DB::table('rrhh.rrhh_beneficios')->insertGetId(
                    [
                        'id_empresa'                    => $empresa,
                        'id_tipo_planilla'              => $planilla,
                        'periodo'                       => $periodo,
                        'mes'                           => $mes,
                        'id_trabajador'                 => $trabajador[$i],
                        'importe_cts'                   => $imp_cts[$i],
                        'importe_grati'                 => $imp_grati[$i],
                        'importe_vacaciones'            => $imp_vacac[$i],
                        'importe_bonificacion'          => $imp_bonif[$i],
                        'importe_descuento'             => $imp_desct[$i],
                        'estado'                        => 1,
                        'fecha_registro'                => $fecha_registro
                    ],
                    'id_beneficio'
        ...
        return redirect()->back()->with('success', 'Se registraron los beneficios correctamente');   
    }
    ...
    function calcularDiasEfectivos($mes, $anio, $fechaIni, $fechaFin, $diasTotal){
        ...
        if ($fechaIni <= $diaIni){
            ...
            if ($fechaIni < $diaFin){ //Si ingresó, esta dentro del rango
                $preFinal = $this->restaFechasDias($diaFin, $fechaIni);
                if ($preFinal < 15) {
                    $nro_dia = $this->filtrar_dia($fechaIni);
                    if ($nro_dia >= 3) {
                        $calFecIni = date('Y-m-d', strtotime($fechaIni));
                        $final = $this->restaFechasDias($diaFin, $calFecIni.'- 1 days');
                    }else{
                        if ($diasTotal > 30) {
                            $calFecIni = date('Y-m-d', strtotime($fechaIni.'- 2 days'));
                            $final = $this->restaFechasDias($diaFin, $calFecIni);
                        }else{
                            $calFecIni = date('Y-m-d', strtotime($fechaIni));
                            $final = $this->restaFechasDias($diaFin, $calFecIni.'- 1 days');
