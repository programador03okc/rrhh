    ...
    $.ajax({
        ...
        success: function(response){
            if (response.id_trabajador > 0){
                ...
                imprimir(response.id_vacaciones, response.id_empresa);
...
function imprimir(id, emp){
    window.open('generar_vacaciones/' + id + '/' + emp, 'Vacaciones', 'height=700,width=800,scrollTo,resizable=1,scrollbars=1,location=0');
    return false;
}